...
Let $x$ be the PSM score and assume that larger score values indicate a better match to the theoretical spectrum, e.g. in case the MS-GF+ score defined in Section \ref{MSGFplusSection}.
...
We assure a monotonic increase of FDR estimates with decreasing score values by replacing the FDR at a particular score $x$ by the maximum TDA FDR in the interval $[x,+\infty]$.
...
In many cases, the set of decoys in the complete search seems a good candidate set.
...
The ECCDF $\bar F_0(x)$ will be zero for every target PSM with a score higher then the maximum decoy PSM score, inducing a FDR estimate of zero for these target PSMs.
...
Lastly, we assure a monotonic increase of FDR estimates with decreasing score values by replacing the FDR at a particular score $x$ by the maximum TDA FDR in the interval $[x,+\infty]$.
...
1. FDR_BH: Use this when you have a large decoy set but no decoy information on the subset PSMs (e.g. when the search engine does not return decoy protien ids). This FDR estimate is more conservative then **FDR** and **FDR_BH**.
